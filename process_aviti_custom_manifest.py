...
def main():
    ...
    custom_manifest = sys.argv[2]
    
    run_bases2fastq(run_folder,custom_manifest)
...
def run_bases2fastq(run_folder,custom_manifest):

    try:
        if os.path.exists(run_folder):
            if os.path.exists(f"{run_folder}/RunUploaded.json"):
                # currently running this from within the AV240405 folder
                print(f"running bases2fastq on run_folder = {run_folder} using custom_manifest = {custom_manifest}.........")
            else:
        ...
        #bases2fastq_cmd = f"bases2fastq -p 16 --run-manifest /data/AV240405/{custom_manifest} {run_folder} {run_folder}/Unaligned"
        bases2fastq_cmd = f"bases2fastq -p 16 --run-manifest {custom_manifest} {run_folder} {run_folder}/Unaligned"
