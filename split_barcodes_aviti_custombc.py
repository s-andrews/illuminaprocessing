...
# passing in the lane number worked in that the splitting worked, but the naming went a bit wrong
# we ended up with the big lane number after L00 instead of 1 or 2
# lane8891_AGAGTAGC_TACGCCTT_Library7_sample11_L008891_R1.fastq.gz - should have been L001
# lane8892_CAGTCCAA_GATTACCG_Library5_D7_PT_L008892_R1.fastq.gz - should have been L002
...
def split_fastqs(file_location, expected_barcodes, double_coded, barcode_length, i1_umi, path_from_run_folder, lane):
    ...
    for key in expected_barcodes:
        ...
        new_filenameR1 = f"lane{lane}_{key}_{expected_barcodes[key]}_L00{lane}_R1.fastq.gz"
        open_filehandlesR1(new_filenameR1, key, path_from_run_folder)

        if paired_end:
            new_filenameR2 = f"lane{lane}_{key}_{expected_barcodes[key]}_L00{lane}_R2.fastq.gz"
            open_filehandlesR2(new_filenameR2, key, path_from_run_folder)

    # also open an unassigned file
    new_filenameR1 = f"lane1_NoCode_L00{lane}_R1.fastq.gz"
    open_filehandlesR1(new_filenameR1, "unassigned", path_from_run_folder)
    new_filenameI1 = f"lane1_NoCode_L00{lane}_I1.fastq.gz"
    open_filehandlesR1(new_filenameI1, "unassigned_I1", path_from_run_folder)

    if double_coded:
        new_filenameI2 = f"lane1_NoCode_L00{lane}_I2.fastq.gz"
        open_filehandlesR1(new_filenameI2, "unassigned_I2", path_from_run_folder)    

    if paired_end:
        new_filenameR2 = f"lane1_NoCode_L00{lane}_R2.fastq.gz"
...
def get_expected_barcodes_sample_sheet(run_folder, lane_number, sample_sheet):
    ...
    # with open(sample_sheet, "r") as ss:
    #     row = ss.readline()
    #     print(row)

    #ss = open(sample_sheet, 'r', newline = None)
