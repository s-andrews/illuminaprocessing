...
def run_bases2fastq(run_folder):
    ...
    try:
        ...
        bases2fastq_cmd = f"bases2fastq -p 16 --r2-cycles 0 --run-manifest ~/illuminaprocessing/aviti_run_manifest_filter_mask.csv {run_folder} {run_folder}/Unaligned --filter-mask R2:N*"
...
def get_barcodes_I1(run_folder, n_bars_to_check):
    ...
    try:
        ...
        bc_check_cmd = f"zcat lane1_NoIndex_L001_I1.fastq.gz | head -n 400000 | awk 'NR % 4 == 2' | sort | uniq -c | sort -k 1 -n -r | head -n {n_bars_to_check} > top_{n_bars_to_check}_barcodes_found_L001_I1.txt"
...
def get_barcodes_I2(run_folder, n_bars_to_check):
    ...
    try:
        os.chdir(f"/primary/{run_folder}/Unaligned/Project_External/Sample_lane1/")
        bc_check_cmd = f"zcat lane1_NoIndex_L001_I2.fastq.gz | head -n 100000 | awk 'NR % 4 == 2' | sort | uniq -c | sort -k 1 -n -r | head -n {n_bars_to_check} > top_{n_bars_to_check}_barcodes_found_L001_I2.txt"
